...
def insert_data(conn, data):
    query = '''
    INSERT INTO articoli (Descrizione, Categoria, Marca, Ean, Pcs_crt, Crt_plt, Hs_code, Data_creazione, Data_modifica)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    '''
...
def update_data(conn, data, article_id):
    query = '''
    UPDATE articoli
    SET Descrizione = ?, Categoria = ?, Marca = ?, Ean = ?, Pcs_crt = ?, Crt_plt = ?, Hs_code = ?, Data_modifica = ? 
    WHERE Id_art = ?
    '''
...
def main():
    ...
    with tab1:
        ...
        with st.form(key="frm_Crea", clear_on_submit=True):
            ...
            Cr_Ctg = st.text_input("Categoria", key="Cr_ctg")
            ...
            Cr_Hsc = st.text_input("HS Code", key="Cr_Hsc")
        ...
        if Cr_Submit:
            if len(Cr_Dsc) < 3:
                ...
                data = (Cr_Dsc, Cr_Ctg, Cr_Mrc, Cr_Ean, Cr_Pcs, Cr_Crt, Cr_Hsc, Cr_DataCreazione, Cr_DataModifica)
    ...
    with tab2:
        ...
        with st.form(key="frm_Modifica", clear_on_submit=True):
            ...
            if article_details['Categoria'] is not None:
                Md_Ctg = st.text_input("Categoria", value=article_details['Categoria'], key="Md_ctg")
            else:
                Md_Ctg = st.text_input("Categoria", key="Md_ctg")
            ...
            if article_details['Pcs_Crt'] is not None:
                ...
                Md_Pcs = st.text_input("PZ / CRT", key="Pcs_Crt")
            if article_details['Crt_Plt'] is not None:
                ...
                Md_Crt = st.text_input("CRT / PLT", key="Crt_Plt")
            if article_details['Hs_code'] is not None:
                Md_Hsc = st.text_input("HS Code", value=article_details['Hs_code'], key="Md_Hsc")
            else:
                Md_Hsc = st.text_input("HS Code", key="Hs_code")
        ...
        if Update_Submit:
            if len(Md_Dsc) < 3:
                ...
                data = (Md_Dsc, Md_Ctg, Md_Mrc, Md_Ean, Md_Pcs, Md_Crt, Md_Hsc, Md_DataModifica)
